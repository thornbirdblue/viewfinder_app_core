...
const DBRegisterKeyIntrospect kEpisodeSelectionKeyIntrospect(
    kEpisodeSelectionKeyPrefix, nullptr, [](Slice value) {
...
class QueryRewriter : public FullTextQueryVisitor {
 ...
 private:
  void AddContactTerms(const string& query, bool prefix, vector<FullTextQuery*>* terms) {
    ...
    state_->contact_manager()->Search(query, &contact_results, nullptr, options);
...
bool EpisodeTable_Episode::GetLocation(Location* loc, Placemark* pm) {
  ...
  location_.reset(nullptr);
  placemark_.reset(nullptr);
  return false;
}
...
void EpisodeTable::RemovePhotos(
  ...
  for (int i = 0; !episodes.empty() && i < photo_ids.size(); ++i) {
    ...
    ActivityMetadata::Episode* e = nullptr;
...
void EpisodeTable::SaveContentHook(Episode* episode, const DBHandle& updates) {
  ...
  if (episode->CountPhotos() > 0) {
    ...
    if (episode->GetLocation(&loc, &pm)) {
      ...
      IndexLocationTerm(FormatPlacemarkWithReferencePlacemark(pm, nullptr, false, PM_SUBLOCALITY, 2));
      IndexLocationTerm(FormatPlacemarkWithReferencePlacemark(pm, nullptr, false, PM_LOCALITY, 2));
      IndexLocationTerm(FormatPlacemarkWithReferencePlacemark(pm, nullptr, false, PM_STATE, 2));
    }
