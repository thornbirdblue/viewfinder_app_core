...
bool DBMigration::MaybeMigrate() {
  ...
  RunMigration(kRemoveFeedEventDataKey,
               ...
               updates);
  RunIOSMigration("7.0", nullptr, kRemoveLocalOnlyPhotosKey,
                  &DBMigration::RemoveLocalOnlyPhotos,
                  updates);
  RunIOSMigration(nullptr, "7.0", kConvertAssetFingerprintsKey,
                  &DBMigration::ConvertAssetFingerprints,
                  updates);
  RunIOSMigration(nullptr, nullptr, kIndexPhotosKey,
  ...
  RunMigration(kPrepareViewpointGCQueueKey,
               ...
               updates);
  RunIOSMigration(nullptr, nullptr, kRemoveAssetDuplicatePhotosKey,
...
void DBMigration::AssetFingerprintIndex(const DBHandle& updates) {
  for (DB::PrefixIterator iter(updates, DBFormat::deprecated_asset_reverse_key(""));
    ...
    const Slice value = iter.value();
    Slice fingerprint;
    if (!DecodeDeprecatedAssetReverseKey(key, &fingerprint, nullptr)) {
      continue;
    }
